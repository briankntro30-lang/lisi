...
# ===================================================
# DATABASE — ruta fija de Windows
# ===================================================

DB_PATH = r"C:\Users\BFRANCOCANTERO\Downloads\Data\simogramme_data.db"

# Si no existe la carpeta, crearla; si la ruta no es accesible (ej. en otro OS), usar home
try:
...
# "loaded_config" holds a config row to restore on next render
...
# ===================================================
# When the user clicks "Charger", we store the config in session_state["loaded_config"]
# and rerun. On the next render (here), we push values into widget keys then clear it.

if st.session_state["loaded_config"] is not None:
    cfg = st.session_state["loaded_config"]
    # col indices: 0=id,1=date,2=of,3=ref,4=machine,5=pdc,6=vc,7=vf,
    #              8=hab,9=act,10=cond,11=stab,12=ja,13=repo,14=h,15=machines,16=donnees,17=res
    ...
    st.session_state["habilete"]    = float(cfg[8])
    st.session_state["activite"]    = float(cfg[9])
    st.session_state["conditions"]  = float(cfg[10])
    st.session_state["stabilite"]   = float(cfg[11])
    st.session_state["repo"]        = float(cfg[13])
    st.session_state["heures"]      = float(cfg[14])

    # Restore machines list
    try:
    ...
    # Restore table data per machine into "preload_table_M1" etc.
    try:
    ...
    st.session_state["loaded_config"] = None  # clear after applying
...
if st.session_state["show_history"]:
    ...
    if configurations:
        for cfg in configurations:
            ...
            pdc_val = cfg[5] or "—"
            mach_val= cfg[4] or "—"
            label   = f"OF: {of_val}  |  PDC: {pdc_val}  |  Machine: {mach_val}  |  {cfg[1][:16]}"
            with st.expander(label):
                col_info, col_btn = st.columns([3, 1])
                with col_info:
                    st.write(f"**Référence pièce:** {cfg[3]}  |  **Vc:** {cfg[6]}  |  **Vf:** {cfg[7]}")
                    st.write(f"**Coef JA:** {float(cfg[12]):.2f}  |  **Coef REPO:** {cfg[13]}  |  **Heures/j:** {cfg[14]}")
                    try:
                        res = json.loads(cfg[17])
                        st.write(f"**Temps cycle:** {round(res.get('temps_cycle_final',0),2)} s  |  "
...
dfs_for_save   = []   # used only when saving / generating
tables_output  = {}   # editor output per machine
...
if st.button("Générer le simogramme"):
    ...
    # Save data — built from edited_df (already captured above before button click)
    save_data = {
